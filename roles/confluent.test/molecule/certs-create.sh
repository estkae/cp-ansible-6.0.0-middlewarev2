...
{{ confluent_jdk_home }}/keytool -noprompt -import \
...
openssl genrsa \
    -out {{ confluent_ssl }}/private/generation/tokenKeypair.pem 2048

echo "  >>>  Create MDS Public Key"
openssl rsa -in {{ confluent_ssl }}/private/generation/tokenKeypair.pem \
 -outform PEM -pubout -out {{ confluent_ssl }}/private/generation/public.pem
...
for line in `sed '/^$/d' $filename`; do
    ...
    read -ra split_hostnames <<< "$line"
      ...
      {{ confluent_jdk_home }}/keytool -genkey -noprompt \
      ...
      {{ confluent_jdk_home }}/keytool -certreq \
          -keystore $KEYSTORE_FILENAME \
          -alias $alias \
    			-ext $EXT \
          -file $CSR_FILENAME \
